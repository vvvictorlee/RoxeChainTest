...
function calcOutGivenIn(tokenBalanceIn, tokenWeightIn, tokenBalanceOut, tokenWeightOut, tokenAmountIn, swapFee) {
    ...
    var  y = Decimal(Decimal(tokenBalanceIn).div(Decimal(tokenBalanceIn).plus(adjustedIn))).toFixed(ONE_DECIMAL);
    var  foo = Decimal(y).pow(weightRatio);
...
function calcInGivenOut(tokenBalanceIn, tokenWeightIn, tokenBalanceOut, tokenWeightOut, tokenAmountOut, swapFee) {
    ...
    var  foo = Decimal(Decimal(y).pow(weightRatio).minus(Decimal(1))).toFixed(11);
    var  tokenAmountIn = Decimal((Decimal(tokenBalanceIn).times(foo)).div(Decimal(1).minus(Decimal(swapFee)))).toFixed(11);
...
function getTransferFee(amount, symbolcode, is_in) {
    ...
    var fee_amount = Number(st.fee) + Number(amount) * Number(st.percent) / Number(percent_decimal);
    if (is_in) {
...
function refactoringObj(arr) {
    return arr.map(function (obj) {
        ...
        o[s[1].trim()] = obj;
        return o;
    }).reduce(function (obj, o) { return Object_assign(obj, o); }, {});
}
...
function convert_one_decimals(amount, token, signed_one ) {
       signed_one=  signed_one||1;
...
function sell(tokenAmountIn, tokenIn, tokenOut) {
    ...
    return Number(Decimal(Decimal(expected * d).floor(0) / d));
}
