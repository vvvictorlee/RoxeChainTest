...
// const dodomath = require("debug")('dodomath');

const debug = require("debug");
const dodomath = debug('dodomath');
...
export class DODOMath {
    ...
    */
    static _GeneralIntegrate(
        ...
        dodomath("_GeneralIntegrate==i, V1, V2=", i, V0, V1, V2);
        ...
        dodomath("==mmm, mmm1, mmm0, ===", mmm0, mmm1, mmm);
        ...
        dodomath("_GeneralIntegrate===", fairAmount, V0V0V1V20, V0V0V1V21, V0V0V1V2, penalty);
        ...
        dodomath("_GeneralIntegrate===", r0, r1, r);
    ...
    */
    static _SolveQuadraticForTrade(
        ...
        dodomath("==_SolveQuadraticForTrade==", ideltaB, deltaBSig, k, Q0, Q1);
        ...
        dodomath("==_SolveQuadraticForTrade=73=", b, kQ02Q1);
        ideltaB = ideltaB;
        if (deltaBSig) {
        ...
        dodomath("==_SolveQuadraticForTrade=80=", b, kQ02Q1);

        if (Number(b) >= Number(kQ02Q1)) {
            dodomath("==_SolveQuadraticForTrade=b >= kQ02Q1=", minusbSig, b, kQ02Q1);
        ...
        dodomath("==_SolveQuadraticForTrade=88=", minusbSig, b, kQ02Q1);
        ...
        dodomath("mm1,mm2===", mmm, mm1, mm2);

        let squareRootv1: number = mmm;//Decimal(DecimalMath.mul(squareRootv0,squareRootv01));//.floor(0); // 4(1-k)kQ0^2
        dodomath("==_SolveQuadraticForTrade=squareRootv1=", squareRootv0, squareRootv010, squareRootv01, squareRootv1);
        ...
        dodomath("(numerator,==== denominator)", squareRoot, minusbSig, numerator, denominator);
    ...
    */
    static _SolveQuadraticForTarget(
        ...
        dodomath("V1, k, fairAmount==", V1, k, fairAmount);
        // V0 = V1+V1*(sqrt-1)/2k
        ...
        dodomath("===sq, sqrtv0,  sqrtv1,sqrtv, sqrt, premium=", sq, sqrtv0, sqrtv, sqrt, premium);
        // V0 is greater than or equal to V1 according to the solution
        const r = Decimal(DecimalMath.mul(V1, DecimalMath.ONE.add(premium))).floor(0);
        dodomath("r= ", r);
        // premium=1.254822;
        //dodomath("DecimalMath.mul(V1, DecimalMath.ONE.add(premium))", DecimalMath.mul(V1, Decimal(DecimalMath.ONE).add(Decimal(premium*DecimalMath.ONE).floor(0))));
        return r;
    }
}
